#!/usr/bin/env python3
"""
Generate Kudo Advisory Mini Brand Guidelines v5 (updated palette).
Adds champagne gold + muted teal to the navy + cream system.
Output: Kudo_Advisory_Mini_Guidelines_v5.pdf
"""
...
OUT = os.path.join(ROOT, "Kudo_Advisory_Mini_Guidelines_v5.pdf")
...
GOLD_INK = HexColor("#88632B")   # antique gold: accent/text on warm cream
...
c.drawString(22 * mm, H - 53 * mm, "Mini Brand Guidelines  (v5, mixed dark + warm system)")
...
for line in [
    "v5 documents the mixed system now live on the website: dark navy bands",
    "alternate with warm cream reading bands. One gold accent, expressed two",
    "ways: champagne on dark surfaces, antique gold on cream. Teal supports.",
]:
    text.textLine(line)
c.drawText(text)
...
rows = [
    ...
    ("Champagne Gold", "#C7A35A", "RGB 199, 163, 90", "The accent ON DARK surfaces: CTAs, highlights, eyebrows on navy.", GOLD, NAVY),
    ("Antique Gold", "#88632B", "RGB 136, 99, 43", "The SAME accent ON CREAM: text, labels, buttons in warm bands (WCAG AA).", GOLD_INK, CREAM),
...
c.drawString(22 * mm, y - 2 * mm, "The band system (rule of thumb)")
c.setFillColor(MUTE); c.setFont("Helvetica", 9.5)
c.drawString(22 * mm, y - 9 * mm, "Dark bands: hero, quotes, testimonials, contact, footer. Warm cream bands: reading content.")
c.drawString(22 * mm, y - 15 * mm, "One gold, two expressions: champagne #C7A35A on navy; antique #88632B on cream. Never swap them.")
c.drawString(22 * mm, y - 21 * mm, "Warm band ink: navy #10222E headings, warm grey #5F5A50 body, off-white #FCFAF3 cards.")
c.showPage()
...
c.drawString(22 * mm, 18 * mm, "Kudo Advisory  |  Mini Brand Guidelines v5  |  Confidential")
c.showPage()
